<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-input @keyup.enter.native="handleFilter" clearable style="width: 200px;" class="filter-item" placeholder="标题" v-model="listQuery.healthyTitle">
      </el-input>
    ...
    <el-table :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="120px" align="center" label="标题">
        <template slot-scope="scope">
          <span>{{scope.row.healthyTitle}}</span>
        </template>
      </el-table-column>

      <el-table-column width="140px" align="center" label="作者">
        <template slot-scope="scope">
          <span>{{scope.row.healthyAuthor}}</span>
        </template>
      </el-table-column>

      <el-table-column width="180px" align="center" label="发布日期">
        <template slot-scope="scope">
          <span>{{scope.row.healthyCreateTime | parseTime('{y}-{m}-{d} {h}:{i}')}}</span>
        </template>
      </el-table-column>

      <el-table-column width="100px" align="center" label="阅读量">
        <template slot-scope="scope">
          <span>{{scope.row.healthyViewCount}}</span>
        </template>
      </el-table-column>

      <el-table-column width="auto" align="left" label="内容">
        <template slot-scope="scope">
          <span style="width:300px;text-overflow:ellipsis; white-space:nowrap; overflow:hidden;">{{scope.row.healthyContent}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="操作" width="207">
        <template slot-scope="scope">
          ...
          <router-link :to="'/healthy/editHealthy/'+scope.row.id">
...
<script>
  import { healthyList, deleteById } from '@/api/healthy'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        healthyList(this.listQuery).then(response => {
      ...
      deleteById(id) {
        this.$confirm('此操作将永久删除文章 ' + id + '  , 是否继续?', '提示', { type: 'warning' })
          .then(() => {
            deleteById(Object.assign({}, { 'healthyId': id })).then(response => {
              this.$notify({
                title: '成功',
                message: '删除文章成功',
                type: 'success',
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['ID', '健康文章标题', '健康文章作者', '发布日期', '阅读量']
          const filterVal = ['id', 'healthyTitle', 'healthyAuthor', 'healthyCreateTime', 'healthyViewCount']
          const data = this.formatJson(filterVal, this.list)
          excel.export_json_to_excel({
            header: tHeader,
            data,
            filename: 'healthy-list'
          })
